...
export function Admin() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.warn("[Firestore] Failed to load issues in Admin (billing expired fallback):", err);
      setIssues(MOCK_ISSUES);
      if (MOCK_ISSUES.length > 0 && !selectedIssueId) {
        setSelectedIssueId(MOCK_ISSUES[0].issue_id);
      }
    });
    return () => unsubscribe();
  }, [selectedIssueId]);
